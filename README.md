...
| [**Capture**](https://github.com/singlouddotorg/Simple-Minutes) | A phone-sized logger: page numbers only, no names. Its files import straight into Minutes and Compile. | 1.0 release |
| **Compile** | This app. | 1.0 release |
...
The everyday case: a singing already logged in Capture (or a Master CSV from the full
...
Maker/SHMHA/Simple List builders), so anything Compile produces will always read the
...
correcting — a mistyped page, a name to fix — open it in the full [Minutes](https://github.com/singlouddotorg/Secretary)
app instead; Compile's own "Export .csv" isn't offered here on purpose (see
"Copy and Share" below), but the file it read is untouched and opens there just as it is.

Leaderless files — Capture's everyday output — are the case this app is built around:
...
1. Open `index.html` and drop in a CSV — a Capture export, or a Master CSV from
   Minutes.
...
Every other app in this Suite writes files. Compile deliberately doesn't: the point
...
Compile keeps no project between visits and writes nothing to browser storage in
...
needs revisiting later, keep the original CSV; Compile is a one-way read of it, not a
place to store it.
